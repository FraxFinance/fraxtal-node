...
1. Copy the env file of the network you want to start removing the `.SAMPLE` suffix (e.g. from `.env.hoodi-testnet.SAMPLE` to `.env.hoodi-testnet`)
2. Edit the resulting file as needed (in this case `.env.hoodi-testnet`)
3. Run the netork-specific compose file:

```
docker compose -f hoodi-testnet.docker-compose.yml up
```

### Cleanup

Delete the \<network>/data folder (e.g. `hoodi-testnet/data`) which contains all the stored chain data.
